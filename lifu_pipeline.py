...
def step7b_lens(data, out, S):
    log("[7b] designing focusing lens + writing orientation spec ...")
    aff = S["aff"]
    ...
    F = float(np.linalg.norm(tgt_mm - txc_mm))                    # focal distance (mm)
    R = TX_DIAM_MM / 2.0

    # plano lens whose thickness converts the flat aperture into a wavefront focusing at F.
    # equal transducer->focus travel time for every radius r:  h(r)*(1/c_lens-1/c_med)+d(r)/c_med = const
    ...
    h = (T0 - tt) / inv * 1e3                                     # lens thickness profile (mm)
    h = h - h.min()
    top = LENS_BASE_MM + h
    lens = _revolve(rr, top)
    ...
    shape = "plano-convex (thick centre)" if inv > 0 else "plano-concave (thick rim)"
    spec = (
        f"TRANSDUCER ORIENTATION & LENS SPEC  (target: {S.get('target_name') or 'label ' + str(S.get('target_label',''))})\n"
        "================================================================\n"
        ...
        f"curvature sag             {h.max():.2f} mm  (total thickness {top.min():.2f}..{top.max():.2f} mm)\n"
        f"lens volume               {lens.volume/1e3:.2f} cm^3   watertight={lens.is_watertight}\n\n"
        "note: lens corrects the geometric (water) path only -- no skull-aberration\n"
        "correction yet; focus is soft (see step11). Backing cylinder in the sim makes\n"
        "the source radiate forward only.\n"
    )
    with open(os.path.join(out, "transducer_spec.txt"), "w", encoding="utf-8") as f:
        f.write(spec)
    log(f"    lens.stl  sag {h.max():.2f} mm, {top.min():.2f}-{top.max():.2f} mm thick, "
...
def step10_sim(data, out, S):
    ...
    # absorbing backing: a cylinder just BEHIND the transducer so it radiates forward only
    txc = np.argwhere(tx).mean(0)
    bdir = txc - T; bdir = bdir / (np.linalg.norm(bdir) + 1e-9)      # points away from target (backward)
    ...
    alpha = np.zeros(c.shape, np.float32); alpha[back] = 40.0        # dB/(MHz^y cm): damp back-going waves
...
<canvas id="c"></canvas>
<div id="ui">
 <h1>Focus: __PSTN__ kPa in STN · __PMAX__ kPa peak</h1>
 ...
 <div class="leg"><div class="sw" style="background:#ff26f2"></div>peak spiking point</div>
...
uniform float aspect,dist,pOp,pTh,aOp,stnOp,txOp,clip,mOp;
uniform vec3 peak;
...
void main(){
 ...
 for(int i=0;i<192;i++){
  ...
  float aS=s.b*stnOp,aT=s.a*txOp;
  vec3 col=hot(s.r)*aP+vec3(0.54,0.58,0.66)*aA+vec3(0.16,1.0,0.30)*aS+vec3(0.20,0.80,1.0)*aT;
  float a=clamp(aP+aA+aS+aT,0.,1.);
...
function draw(){const w=cv.clientWidth,h=cv.clientHeight;if(cv.width!==w||cv.height!==h){cv.width=w;cv.height=h;}
 ...
 gl.uniform1f(U('stnOp'),+so.value);gl.uniform1f(U('txOp'),+to.value);gl.uniform1f(U('clip'),+cl.value);
...
const po=document.getElementById('po'),pt=document.getElementById('pt'),ao=document.getElementById('ao'),
 so=document.getElementById('so'),to=document.getElementById('to'),cl=document.getElementById('cl'),mo=document.getElementById('mo');
function lbl(){vpo.textContent=(+po.value).toFixed(2);vpt.textContent=(+pt.value).toFixed(2);vao.textContent=(+ao.value).toFixed(3);
 vso.textContent=(+so.value).toFixed(2);vto.textContent=(+to.value).toFixed(2);vcl.textContent=(+cl.value).toFixed(2);vmo.textContent=(+mo.value).toFixed(2);}
let raf=0;function schedule(){if(!raf)raf=requestAnimationFrame(()=>{raf=0;draw();});}  // defer heavy draw
[po,pt,ao,so,to,cl,mo].forEach(e=>e.addEventListener('input',()=>{lbl();schedule();}));lbl();
...
def write_viewer(out, S):
    import base64
    p = S["p"]; c = S["c_crop"]; st = S["stn_crop"]; tx = S["tx_crop"]
    ...
    p_stn = float((p * st).max()) / 1e3 if st.any() else 0.0
    pm = p * st if st.any() else p                           # mark the peak inside the STN (else global)
    fk = np.unravel_index(int(np.argmax(pm)), p.shape)
    peak = f"{(fk[0]+0.5)/p.shape[0]:.4f},{(fk[1]+0.5)/p.shape[1]:.4f},{(fk[2]+0.5)/p.shape[2]:.4f}"
    html = (VIEWER_HTML.replace("__NX__", str(Nx)).replace("__NY__", str(Ny)).replace("__NZ__", str(Nz))
            .replace("__PMAX__", f"{p.max()/1e3:.0f}").replace("__PSTN__", f"{p_stn:.0f}")
            .replace("__PEAK__", peak).replace("__DATA__", b64))
...
def step11_view(data, out, S):
    ...
    foc = np.array(np.unravel_index(int(np.argmax(np.where(st, p, 0) if st.any() else p)), p.shape))
    d_mm = float(np.linalg.norm((foc - tgt) * dx_mm))
    ...
    for ax, (fx, nm, a0, a1) in zip(axes, planes):
        ...
        ax.plot(foc[a0], foc[a1], "x", color="lime", ms=11, mew=2.5, label="focal peak")
    ...
    fig.suptitle(f"Transcranial focus on STN  |  {p_stn/1e3:.0f} kPa at target "
                 f"({d_mm:.1f} mm off centroid)  |  global peak {pk/1e3:.0f} kPa",
                 fontsize=13)
    fig.savefig(os.path.join(out, "step11_pressure.png"), dpi=130, bbox_inches="tight")
    plt.close(fig)
    log(f"    peak-in-STN {p_stn/1e3:.1f} kPa at {tuple(int(v) for v in foc)}, {d_mm:.1f} mm from centroid")
    write_viewer(out, S)
    return S
...
def main():
    ...
    if args.run_sim:
        step10_sim(args.data, args.out, S)
        step11_view(args.data, args.out, S)
    else:
